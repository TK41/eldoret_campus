...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form_mode'] ?? '') === 'login') {
    ...
    if (!$username || !$password) {
        ...
        $stmt = $db->prepare("SELECT admin_id,first_name,last_name,full_name,username,password_hash,role,is_active FROM admin_users WHERE username=? LIMIT 1");
        ...
        if (!$admin || !$admin['is_active']) {
            ...
            $db->prepare("UPDATE admin_users SET last_login=NOW() WHERE admin_id=?")->execute([$admin['admin_id']]);
            header('Location: ' . APP_ROOT . '/portal.php');
            exit;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form_mode'] ?? '') === 'signup') {
    ...
    if (empty($errors)) {
        ...
        $role       = ($adminCount == 0) ? 'superadmin' : 'staff';
        $full_name  = trim("$first_name $last_name");
        $db->prepare("INSERT INTO admin_users (first_name,last_name,full_name,email,username,password_hash,role) VALUES (?,?,?,?,?,?,?)")
           ->execute([$first_name, $last_name, $full_name, $email, $username, password_hash($password, PASSWORD_BCRYPT), $role]);
